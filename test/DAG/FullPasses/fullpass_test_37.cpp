...
int main( int argc, char **argv ){
  ...
  // insert a pseudo inst
  CoreGenPseudoInst *PInst = CG->InsertPseudoInst( PROJNAME + "pinst1",
                                                   Inst );
  if( !PInst->SetEncoding( "RA", 0x0A ) ){
    std::cout << "ERROR : FAILED TO SET ra ENCODING FOR PSEUDO INSST: " <<
      PInst->GetName() << " : " << CG->GetErrStr() << std::endl;
    delete CG;
    return -1;
  }
