...
std::string PROJNAME = "TEST33";
std::string PROJROOT = "./";
std::string ARCHROOT = "./";
std::string PROJYAML = "TEST33.yaml";
...
int main( int argc, char **argv ){
  ...
  if( !Inst->SetEncoding( "RA", 0x05 ) ){
    std::cout << "ERROR : FAILED TO SET ra ENCODING FOR INSTRUCTION: " <<
      Inst->GetName() << " : " << CG->GetErrStr() << std::endl;
    delete CG;
    return -1;
  }

  if( !Inst->SetEncoding( "RB", 0x0A ) ){
    std::cout << "ERROR : FAILED TO SET rb ENCODING FOR INSTRUCTION: " <<
      Inst->GetName() << " : " << CG->GetErrStr() << std::endl;
    delete CG;
    return -1;
  }
