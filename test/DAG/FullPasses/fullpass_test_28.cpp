...
int main( int argc, char **argv ){
  ...
  // add a cache
  CoreGenCache *Cache1 = CG->InsertCache( PROJNAME + ".L1.cache", 1, 1 );
  ...
  CoreGenCache *Cache2 = CG->InsertCache( PROJNAME + ".L2.cache", 2, 8 );
  if( !Cache1->SetChildCache( Cache2 ) ){
    std::cout << "ERROR : FAILED TO SET CHILD CACHE LEVEL:" <<
      CG->GetErrStr() << std::endl;
    delete CG;
    return -1;
  }
  if( !Cache2->SetParentCache( Cache1 ) ){
    std::cout << "ERROR : FAILED TO SET PARENT CACHE LEVEL:" <<
      CG->GetErrStr() << std::endl;
    delete CG;
    return -1;
  }
