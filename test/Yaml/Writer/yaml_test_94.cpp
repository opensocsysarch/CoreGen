...
std::string PROJNAME = "TEST94";
std::string PROJROOT = "./";
std::string ARCHROOT = "./";
std::string PROJYAML = "TEST94.yaml";
...
int main( int argc, char **argv ){
  ...
  for( unsigned i=0; i<10; i++ ){
    if( !Stage->InsertInput( "Input" + std::to_string(i)) ){
      std::cout << "ERROR : FAILED TO INSERT VLIW STAGE INPUT: Input"
                << std::to_string(i) << CG->GetErrStr() << std::endl;
      delete CG;
      return -1;
    }
  }
  ...
  for( unsigned i=0; i<10; i++ ){
    if( !Stage->InsertOutput( "Output" + std::to_string(i)) ){
      std::cout << "ERROR : FAILED TO INSERT VLIW STAGE OUTPUT: Output"
                << std::to_string(i) << CG->GetErrStr() << std::endl;
      delete CG;
      return -1;
    }
